type LegalLocale = "de" | "en" | "vi" | "ru" | "uk" | "fa" | "ar" | "sq" | "tr" | "fr" | "es" | "it" | "pl";
type LegalKind = "privacy" | "impressum" | "terms";
type CoreLocale = Exclude<LegalLocale, "vi">;
...
const PRIVACY_CONTENT: Record<LegalLocale, LegalContent> = {
...
const IMPRESSUM_CONTENT: Record<LegalLocale, LegalContent> = {
...
const TERMS_CONTENT: Record<LegalLocale, LegalContent> = {
  ...TERMS_CONTENT_BASE,
  vi: {
...
const FALLBACK_LEGAL_LOCALE: LegalLocale = "en";

const LEGAL_CONTENT_BY_KIND = {
...
export default function LegalPage({ kind, locale, dir }: LegalPageProps) {
  const contentMap = LEGAL_CONTENT_BY_KIND[kind];
  const content = contentMap[locale as LegalLocale] ?? contentMap[FALLBACK_LEGAL_LOCALE];

  return (
    <div className="privacyPage" dir={dir}>
      {renderLegalContent(content.title, content.sections)}
    </div>
  );
}
